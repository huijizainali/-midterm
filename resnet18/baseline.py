...
# Training
writer = SummaryWriter('./path/baseline/log')
def train(epoch):
    print('\nEpoch: %d' % epoch)
    net.train()
    train_loss = 0
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        writer.add_scalar('Loss_train_baseline',loss, epoch)  # tensorboard train loss
        loss.backward()
        optimizer.step()
...
def test(epoch):
    ...
    writer.add_scalar('Acc_test_baseline', acc, epoch)  # tensorboard acc
    if acc > best_acc:
        print('Saving..')
        state = {
            'net': net.state_dict(),
