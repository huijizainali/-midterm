...
# Training
writer = SummaryWriter('./path/cutout/log')
def train(epoch):
    print('\nEpoch: %d' % epoch)
    net.train()
    train_loss = 0
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        writer.add_scalar('Loss_train_cutout',loss, epoch)  # tensorboard train loss
        loss.backward()
        optimizer.step()
...
def test(epoch):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(testloader):
            ...
            outputs = net(inputs)
            loss = criterion(outputs, targets)
            writer.add_scalar('Loss_test_cutout',loss, epoch)
            test_loss += loss.item()
            _, predicted = outputs.max(1)
            total += targets.size(0)
            correct += predicted.eq(targets).sum().item()
    ...
    writer.add_scalar('Acc_test_cutout', acc, epoch)  # tensorboard acc
    if acc > best_acc:
        print('Saving..')
        state = {
            'net': net.state_dict(),
...
def main():
    ...
    # torch.save(val_loss_list, 'cutout_val_loss_list')
    # torch.save(val_acc, 'cutout_val_acc')
