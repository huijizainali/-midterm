...
def get_map(del_predetection,MINOVERLAP, draw_plot, modelname, path = './map_out'):
    plt.switch_backend('agg')
    GT_PATH             = os.path.join(path, 'ground-truth')
    DR_PATH             = os.path.join(path, 'detection-results/'+modelname)
    IMG_PATH            = os.path.join(path, 'images-optional')
    TEMP_FILES_PATH     = os.path.join(path, '.temp_files')
    RESULTS_FILES_PATH  = os.path.join(path, 'results/'+modelname)
    ...
    """
    if draw_plot and not os.path.exists(path+'/results/ground-truth-info.png'):
        window_title = "ground-truth-info"
        plot_title = "ground-truth\n"
        plot_title += "(" + str(len(ground_truth_files_list)) + " files and " + str(n_classes) + " classes)"
        x_label = "Number of objects per class"
        output_path = path + "/results/ground-truth-info.png"
        to_show = False
    ...
    if draw_plot:
        ...
        output_path = RESULTS_FILES_PATH + "/" + modelname + "lamr.png"
        to_show = False
    ...
    if draw_plot:
        ...
        output_path = RESULTS_FILES_PATH + "/" + modelname + "mAP.png"
        to_show = True
        plot_color = 'royalblue'
        draw_plot_func(
            ap_dictionary,
